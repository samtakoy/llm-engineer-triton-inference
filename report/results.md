...
(`bls_rude`) и три модели по отдельности. Шкала логарифмическая. Видно две группы:
...
Другие конфигурации не пробовал.
...
это окупается: он лёгкий, пачка набирается быстро. 

Эмбеддер тяжелее, за 3 мс  набрать пачку не успевает, и запрос отлёживает задержку впустую — 28.1
...
**4. Экономить вызовы генератора, а не ускорять модели.** Путь
